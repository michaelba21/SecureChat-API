...
class AuthControllerTest {
        ...
        class LoginTests {
                ...
                @Test
                @Disabled("Local authentication disabled - migrated to Keycloak OAuth2")
                ...
                // NEW TEST: Verify current behavior - local login disabled returns 501
                @Test
                void login_localAuthDisabled_returns501() throws Exception {
                        // Arrange: create login request
                        LoginRequest request = new LoginRequest("test@example.com", "Password123");

                        // Act & Assert: Local login is disabled, returns 501 Not Implemented
                        mockMvc.perform(post("/api/auth/login")
                                        .principal(mockAuth) // Set authentication principal
                                        .header("X-Forwarded-For", "192.168.1.1") // IP header
                                        .header("User-Agent", "Test-Agent") 
                                        .contentType(MediaType.APPLICATION_JSON) 
                                        .content(objectMapper.writeValueAsString(request))) // Request body as JSON
                                        .andExpect(status().isNotImplemented()) // HTTP 501 - Local login disabled
                                        .andExpect(jsonPath("$.error").value(containsString("disabled"))); // Error message contains "disabled"
                }
        }
        ...
        class RegisterTests {
                ...
                // NEW TEST: Verify current behavior - local registration disabled returns 501
                @Test
                void register_localAuthDisabled_returns501() throws Exception {
                        // Arrange: registration request
                        AuthRequest request = new AuthRequest("newuser", "Password123", "new@test.com");

                        // Act & Assert: Local registration is disabled, returns 501 Not Implemented
                        mockMvc.perform(post("/api/auth/register")
                                        .principal(mockAuth)
                                        .header("X-Forwarded-For", "192.168.1.1")
                                        .header("User-Agent", "Test-Agent")
                                        .contentType(MediaType.APPLICATION_JSON)
                                        .content(objectMapper.writeValueAsString(request)))
                                        .andExpect(status().isNotImplemented()) // HTTP 501 - Local registration disabled
                                        .andExpect(jsonPath("$.error").value(containsString("disabled"))); // Error message contains "disabled"
                }
        }
        ...
        class RefreshTests {
                ...
                // NEW TEST: Verify current behavior - Keycloak handles refresh, returns 501
                @Test
                void refresh_keycloakHandled_returns501() throws Exception {
                        // Arrange: valid refresh token

                        // Act & Assert: Token refresh handled by Keycloak, returns 501 Not Implemented
                        mockMvc.perform(post("/api/auth/refresh")
                                        .principal(mockAuth)
                                        .header("X-Forwarded-For", "192.168.1.1")
                                        .header("User-Agent", "Test-Agent")
                                        .contentType(MediaType.APPLICATION_JSON)
                                        .content("{\"refreshToken\":\"valid-refresh-token\"}")) // JSON payload
                                        .andExpect(status().isNotImplemented()) // HTTP 501 - Keycloak handles refresh
                                        .andExpect(jsonPath("$.error").value(containsString("Keycloak"))); // Error message mentions Keycloak
                }

                // NEW TEST: Verify current behavior - missing refresh token returns 501 (not 400)
                @Test
                void refresh_missingToken_keycloakHandled_returns501() throws Exception {
                        // Act & Assert: Token refresh handled by Keycloak, returns 501 Not Implemented
                        mockMvc.perform(post("/api/auth/refresh")
                                        .principal(mockAuth)
                                        .header("X-Forwarded-For", "192.168.1.1")
                                        ...
                                        .content("{}")) // Empty JSON object
                                        .andExpect(status().isNotImplemented()) // HTTP 501 - Keycloak handles refresh
                                        .andExpect(jsonPath("$.error").exists()); // Error message exists
                }
        }
